...
const functions = {
    ...
    'VBNN__use_a_to_VBNN': function VBNN__use_a_to_VBNN (arr) {
        ...
        function tests (sub) { return [
            K.V.use.test(sub.a.en), sub.b.isNN(), sub.c.en == 'to', sub.d.isVB(), sub.e.isNN()
        ]; }
    ...
    'NN__NN_andor_NN': function NN__NN_andor_NN (arr) {
        ...
        function tests (sub) { return [
            sub.a.isNN(), /^(?:and|or)$/.test(sub.b.en), sub.c.isNN(2)
        ]; }
        function ko (sub) { 
    ...
    'VBNN__VBNN_RB_Clause': function VBNN__VBNN_RB_Clause (arr) {
        ...
        function ko (sub) { return sub.ko(1, 0); }
        return K.loop(arr, affected, tests, 1, 'VBNN', ko, name, 1);
    },
    ...
    'VBNN_VB__single': function VBNN_VB__single (arr) {
        let reg = /^(?:VB|VBNN|Clause|Sentence)$/;
        for (var i = 0; i < arr.length; i++) {
            if (reg.test(arr[i].pos)) {
                let count_VB = arr.filter(o => reg.test(o.pumsa));console.log(count_VB)
                if (count_VB.length == 1) arr[i].state('single');
            }
        }
        return [arr, []];
    },
    ...
    'Sentence__a_be_b': function Sentence__a_be_b (arr) {
        ...
        function ko (sub) { 
            let vi = sub.c.ko + (sub.c.isNN()? '입니다.': '합니다.');
    ...
    'Clause__NN_VBNN': function Clause__NN_VBNN (arr) {
        ...
        function ko (sub) {
            let out = sub.ko(0, 1);
            if (sub.a.isNN()) sub.a.josa('는'); return out; 
        }
        return K.loop(arr, affected, tests, 1, 'Clause', ko, name, 2);
    },
